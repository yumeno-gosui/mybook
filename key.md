...
## TTRPGとは? 
テーブルトップ・ロールプレイング・ゲームでは、1人または複数の友人がキャラクターを作成し、そのキャラクターを操作して、物語を共有する冒険で遊びます。

多くの場合、TTRPGでは、プレイヤーの一人がガイド(通常、ゲームマスターまたはGMと呼ばれる)として、ゲームの語り手や審判の役割を果たす必要があります。

GMはまた、ゲームの架空の設定や冒険の目標を作成する役割も担います。とはいえ、TTRPGは協力プレイやソロプレイも可能であり、GMの責任をプレイヤ ー間で均等に分担したり、サイコロを振ったりカードを引いたりといったランダムに発生 するプロンプトで代用したりすることもできる。 

いずれにせよ、TTRPGではプレイヤーキャラクター（PC）の行動と選択によって冒険の行方が形作られます。PCには様々な属性や技能があり、それらは通常数値で割り当てられ、経歴や職業に結びつきます。

プレイ中、プレイヤーは自分のキャラクターに何をさせたいかを記述し、その行動の成否は通常ダイスを振って決定する。多くのTTRPGでは、プレイヤーはゲーム内の状況やキャラクターの属性や技能によって、結果を加減することもできます。
 
通常のボードゲームとは対照的に、TTRPGは勝つことができません。TTRPGで「勝つ」唯一の方法は、魅力的な冒険を共有し、記憶に残るキャラクターを作り、そして最も重要なことは、楽しむことです！
...
前述したように、ほとんどのTTRPGでは、物語的に重要な行動の成否は、PCの技能の助けを少し借りて、偶然によって決定されます。別のTTRPGでは、このサイコロの目やカードの引きに依存する代わりに、成功の代償としてプレイヤーが消費できるトークンやポイントの数に限りがあるものもあります。

パスキーシステムのゲームでは、プレイヤーは重要な行動ごとに、上記の両方の選択肢を利用することができます。プレイヤーは、自分のキャラクターが成功するために努力するタイミングを選ぶことも、偶然に任せることもできます。

パスキーシステムはまた、キャラクタークリエイション、ワールド構築、ゲームメカニックデザインなど、多くのカスタマイズを可能にするツールを提供します。 

プレイヤーの主体性に焦点を当てたパスキーシステムは、ロールプレイや物語中心のゲーム向けに設計されています。戦闘はパスキーシステムゲームの一部となり得ますが、ルールは戦闘を中心に作られているわけではありませんし、ゲーム内の紛争の解決法として提示されているわけでもありません。パスキー・システムのゲームには数学的な要素はほとんどなく、必要なダイスは古典的な6面ダイス（d6）のみです。
...
### 能力値 
能力値は、PCの属性を表します。パスキーシステムでは以下の使用を推奨する： 
...
各能力値はゲーム中に消費されるエナジーまたは**努力ポイント（EP）**のプールとして機能する。<br> 
PCは4d6分のEPを各能力値に配分した状態でゲームを開始する（7ページ参照）。全ての能力値プールのEPを使い果たした場合、PCは意識不明になるか死亡する（ゲームのトーンによる）。

### アクション 
アクションとは、プレイヤー・キャラクター（PC）がゲーム中に行うことで、失敗する可能性があったり、ゲームにとって重要な結果をもたらすものです。アクションを実行する場合、プレイヤーには常に以下の2つの選択肢があ ります。以下の2つの選択肢があります： 
...
### コンプリケーション 
コンプリケーションとは、アクションの結果発生しうる予期せぬ問 題、挑戦、または障害であり、ゲームの行方を(劇的にでも)変 えることがある。コンプ リケーションの性質はゲーム内で起こっていることに基づいて GM やプ レイヤーが決定することができ、力学的な意味合いや物語的な意味合い、 あるいはその両方を持つこともある。また、ロール表(9 ページ参照)を用いることで、コンプラ イケーションを無作為に生成することもできる。

### アドバンテージとディスアドバンテージ 
状況によってアクションは行いやすくなったり行いにくく なったりする。例えば、そのアクションを行おうとしているPCが助けを受けていたり、便利なアイテムを装備していたりすると、そのアクションはやりやすくなるかもしれませんし、気を取られていたり、体調が悪かったりすると、やりにくくなるかもしれません。より簡単な場合、そのアクションはアドバンテージで実行でき、より困難な場合、ディスアドバンテージで実行できる。

* 有利：EPコストに-1、運の出目に＋1。
* 不利： デメリット：EPコストに+1、運ロールに-1。

アドバンテージとディスアドバンテージはスタックすることができ、 成功するアクションと失敗するアクションがある。

### コンディション 
コンディションとは、特定の状況においてPCに適用される効果のこと で、有害であったり有益であったりし、機械的効果や物語的効果を持 つ。 
以下は推奨されるコンディションのリストである。
 
[活性化］
PCは活性化され、次の休息まで使用可能なボーナスEPを一時的に1d6分得る。
 
阻害
PCの動きがより困難になるか、部分的に拘束される。支障がなくなるまで、支障の影響を受けた行動にはデメリットが生じる。
 
傷害
PCは肉体的、精神的、心理的な負傷を負う。治るまで、傷害（その性質が何であれ） の影響を受けた行動に対して不利益を被る。このコンディションはスタックする。
 
霊感
感動的なことを目撃したり聞いたりした場合、そのPCは次に選択するアクションにアドバンテージを得る。このコンディションはスタックする。
 
[パニック］
冷静さを失い、ストレスに圧倒される。ストレスが最大値以下になるまで、すべてのアクションに不利を得る（ストレスのルールを参照）。
 
[動けなくなる］
PCは完全に動けなくなる。解放されるまで動けなくなる。
 
[意識不明］
意識不明]そのPCはあなたが目を覚ますまで(必要であれば治るまで)、いかなる行動もとることができない。

### 技能と失敗 
スキルとは、経験や訓練、あるいはその両方によってPCが得意とするものである。スキルを使用するアクションにはアドバンテージがあります。

PCは3つのスキルを持ってゲームを開始し、最大8つまで知ることができる。

パスキーシステムにはスキルのリストは付属していません。そのため、プレイヤーの好きなスキルや、ゲームのテーマに合ったスキルを選択することができます。例えば、医学を専攻していればヒーリングや外科手術などのスキルが、戦士のよう なクラスであれば武装戦闘や威嚇などのスキルが選べる。

あなたのゲームに魔法や超能力のような超自然的または幻想的な要素が含まれている 場合は、6ページを参照。 それらの習得を使用可能なスキルとして含めることができる。

フラッブ（Flubs） フラッブ（Flubs）とは、不安や適性の欠如、あるいはその両方によって、PCが不得意 とする事柄のことである。

失敗を伴う行動にはデメリットがある。不器用はプレイヤーが望むものであれば何でもよく（例：人前で話す、料理、値切り交渉など）、PCは2つの不器用を持ってゲームを開始し、最大4つまで持つことができる。

### 特性（オプション） 
あなたのゲームに様々なプレイアブルな種族や民族が登場する場合、特性 を含めることを望むかもしれない。特性は生得的な生物学的または生理学的な特徴であり、機械的な効果 や物語的な効果、あるいはその両方の組み合わせを持つことができる。あなたが作成したプレイアブルな種族や民族のそれぞれについて、特 徴のリストを追加するか、あるいはプレイヤーに特徴を混ぜ合わせ、独自の特 徴を創作させることができます。キャラクターの特性は3つか4つを推奨します。特性の例については9～10ページを参照。

### タイトスポット 
タイトスポットとは、PCがハザードや対戦相手などの脅威に直面しなけ ればならない、危険度の高い状況のことである。相手と対峙する場合、その対峙は物理的なものだけでなく、 言葉によるものであったり、心理的なものであったりする。 

タイト・スポットは手番でプレイされ、脅威のアクションや効果はGMが決定するか、（脅威のレベルに基づいて）1d6表を振って決定する。タイト・スポットを発生させた、または発生させた者が最初のターンを持つ。

ターンの間、すべてのPCは同時に、または連続して行動する。ターン中、各PCは2回までアクションを行なうことができる。アクションには移動、有害なコンディションの発生、コンディションの回避が含まれる。 
脅威が対戦相手であり、傷つけられたり、妨げられたり、動けなくなったりした場合、PCは次のターンの行動でアドバンテージを得る。相手が動けなくなった場合、次の脅威ターンにプレイヤーは1d6を振る。その結果が4以下であった場合、相手はフリーとなり、アクションを実行する。

ハザードが無力化されるか、相手が意識不明、動けなくなるか、死亡した場合、PCの勝利となり、窮地は終了する。
...
オプションのルールとして、緊張感を高めるために、タイトスポッ トの間、すべてのPCのアクションを運ロールとして実行しなければならな いと指定することができる。 

#### ストレス（オプション） 
ゲームの設定やテーマに、PCにとって特にストレスのかかる状況が多く含まれ ている場合、以下のストレスのメカニズムを含めることができる。

窮地に陥っている間、PCはアクションに失敗するたびに1ストレス を得る。 
各PCはパニックになるまでに蓄積できるストレスの最大値を持っています。PCの最大ストレス量は最大知力＋最大スタミナに等しい。
ストレスは特殊アイテムの使用や休息（下記参照）によって軽減することができる。

#### 戦闘による損害(オプション) 
PCがゲーム内で誰かに重傷を負わせたり殺したりするたびに、そのPCの最大社交性スタットが1EP減少する。
